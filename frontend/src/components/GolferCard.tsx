...
export function GolferCard({ golfer, selected, alreadyUsed, alreadyTeedOff, onClick }: Props) {
  const base =
    "flex items-center gap-3 px-4 py-3 rounded-xs border cursor-pointer transition-colors";
  ...
  const style = disabled
    ? `${base} cursor-not-allowed border-ink-200 bg-ink-50`
    : selected
    ? `${base} border-fairway-600 bg-fairway-50 ring-2 ring-fairway-500`
    : `${base} border-ink-200 bg-white hover:border-fairway-400 hover:bg-fairway-50`;
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className={`font-medium truncate ${disabled ? "text-ink-400" : "text-ink-900"}`}>
          {golfer.name}
        </p>
        {golfer.country && (
          <p className="text-xs text-ink-400 truncate">{golfer.country}</p>
        )}
      </div>

      {alreadyUsed && (
        <span className="text-xs font-semibold px-2 py-0.5 rounded-xs bg-ink-200 text-ink-500 shrink-0">
          Used
        </span>
      )}
      {alreadyTeedOff && !alreadyUsed && (
        <span className="text-xs font-semibold px-2 py-0.5 rounded-xs bg-brass-100 text-brass-700 shrink-0">
          Teed off
        </span>
      )}
      {selected && !disabled && (
        <span className="w-5 h-5 rounded-full bg-fairway-500 flex items-center justify-center shrink-0">
