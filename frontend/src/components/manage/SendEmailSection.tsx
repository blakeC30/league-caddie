  ...
  return (
    <section className="bg-white rounded-sm border border-ink-200 p-6 space-y-4">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-base font-bold text-ink-900">Send Email</h2>
      </div>
      ...
      {success && (
        <div className="bg-fairway-50 border border-fairway-200 rounded-xs px-4 py-3 text-sm text-fairway-700">
          Email sent to {success.count} member{success.count !== 1 ? "s" : ""}.
        </div>
      )}

      {error && (
        <div className="bg-flag-50 border border-flag-300 rounded-xs px-4 py-3 text-sm text-flag-600">
          {error}
        </div>
      )}
      ...
      <form onSubmit={handleSend} className="space-y-4">
        ...
        <div className="text-sm text-ink-500">
          Sending to <span className="font-medium text-ink-700">{optedIn.length} member{optedIn.length !== 1 ? "s" : ""}</span>
          {optedOutCount > 0 && (
            <span className="text-ink-400"> ({optedOutCount} opted out)</span>
          )}
        </div>
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-ink-400 tabular-nums">{subject.length}/100</span>
          </div>
          <input
            type="text"
            value={subject}
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-ink-400 tabular-nums">{body.length}/5000</span>
          </div>
          <textarea
            value={body}
            ...
            rows={5}
            className="w-full border border-ink-300 rounded-xs px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-fairway-500 resize-y"
            required
          />
        </div>
        ...
        <button
          ...
          className="w-full bg-fairway-700 hover:bg-fairway-700 disabled:opacity-40 text-white font-semibold py-2.5 rounded-xs transition-colors"
        >
          ...
          {showHistory && (
            <div className="mt-2 space-y-1.5">
              {emailHistory.slice(0, 5).map((e) => (
                <div key={e.id} className="text-xs text-ink-500 flex items-center justify-between">
