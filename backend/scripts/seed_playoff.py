...
Testing workflow:
  ...
  4. Simulate tournament start (picks still hidden — no tee times yet):
       docker compose exec db psql -U postgres league_caddie_dev -c \\
...
def seed_playoff():
    ...
    try:
        ...
        # ---------------------------------------------------------------
        existing = (
            db.query(PlayoffConfig).filter_by(league_id=league.id, season_id=season.id).first()
        )
        if existing:
        ...
        players = (
            db.query(Tournament)
            .filter(Tournament.name.ilike("%PLAYERS%"))
        ...
        lt = (
            ...
            .filter_by(league_id=league.id, tournament_id=players.id)
            .first()
        )
        if not lt:
        ...
        # ---------------------------------------------------------------
        membership = db.query(LeagueMember).filter_by(league_id=league.id, user_id=blake.id).first()
        if not membership:
            db.add(
                LeagueMember(
                    league_id=league.id,
...
def _print_summary(db, league: League, config: PlayoffConfig):
    ...
    round1 = db.query(PlayoffRound).filter_by(playoff_config_id=config.id, round_number=1).first()

    print(f"\n  League:     '{league.name}'  id={league.id}")
    print(f"  Playoff:    config id={config.id}  status={config.status}")
    print(
        f"  Round 1:    id={round1.id if round1 else '?'}  tournament={players.name if players else '?'}  status={round1.status if round1 else '?'}"
    )
    print()
    ...
    print(
        f"    POST /leagues/{league.id}/playoff/rounds/{round1.id if round1 else '<round_id>'}/open"
    )
    print()
    print("  SQL shortcuts:")
    if players:
        print("    -- Set tournament in_progress (picks hidden):")
        print(f"    UPDATE tournaments SET status='in_progress' WHERE id='{players.id}';")
        print("    -- Reveal picks (all R1 teed off):")
        print("    UPDATE tournament_entry_rounds SET tee_time = NOW() - INTERVAL '3 hours'")
        print("      WHERE round_number=1 AND tournament_entry_id IN (")
        print(f"        SELECT id FROM tournament_entries WHERE tournament_id='{players.id}');")
        print("    -- Reset tournament to scheduled:")
