...
def create_league(
    ...
    league = League(
        ...
        auto_accept_requests=body.auto_accept_requests,
        created_by=current_user.id,
        is_admin_league=is_admin,
        has_active_purchase=is_admin,  # admin leagues are always active
...
def preview_join(
    ...
    return LeagueJoinPreview(
        ...
        auto_accept_requests=league.auto_accept_requests,
    )
...
def request_to_join(
    ...
    # Auto-accept or public leagues skip the approval step — check member cap first.
    should_auto_approve = league.is_public or league.auto_accept_requests
    if should_auto_approve:
        ...
        # Determine the member limit from the league purchase.
        from app.models.league_purchase import LeaguePurchase

        lp = (
            db.query(LeaguePurchase)
            .filter_by(league_id=league.id, season_year=datetime.date.today().year)
            .first()
        )
        member_limit = lp.member_limit if lp else LEAGUE_MEMBER_CAP
        if approved_count >= member_limit:
            raise HTTPException(
                status_code=422,
                detail="This league is currently full and not accepting new members. "
                "Contact the league manager to request a spot.",
            )
...
def update_league(
    ...
    # --- Auto-accept toggle ---
    if body.auto_accept_requests is not None:
        # Turning ON: validate that batch-accepting all pending requests won't
        # exceed the league's member limit.
        if body.auto_accept_requests and not league.auto_accept_requests:
            pending_count = (
                db.query(LeagueMember)
                .filter_by(league_id=league.id, status=LeagueMemberStatus.PENDING.value)
                .count()
            ...
            member_limit = lp.member_limit if lp else 500
            total_after = approved_count + pending_count
            if total_after > member_limit:
                raise HTTPException(
                    status_code=422,
                    detail=(
                        f"Cannot enable auto-accept: accepting all {pending_count} pending "
                        f"request{'s' if pending_count != 1 else ''} would bring the league to "
                        f"{total_after} members, exceeding your plan limit of {member_limit}. "
                        "Deny some pending requests or upgrade your plan first."
                    ),
                )
            # Batch-accept all pending requests.
            if pending_count > 0:
                db.query(LeagueMember).filter_by(
                    league_id=league.id, status=LeagueMemberStatus.PENDING.value
                ).update({"status": LeagueMemberStatus.APPROVED.value}, synchronize_session="fetch")

        league.auto_accept_requests = body.auto_accept_requests
