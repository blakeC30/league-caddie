...
class PlayoffConfig(Base):
    ...
    playoff_size: Mapped[int] = mapped_column(
        Integer, nullable=False, default=16, server_default="16"
    )
    draft_style: Mapped[str] = mapped_column(
        String(30), nullable=False, default="snake", server_default="snake"
    )
    picks_per_round: Mapped[list[int]] = mapped_column(
        JSON, nullable=False, default=lambda: [2, 2], server_default="[2,2]"
    )
    ...
    rounds: Mapped[list["PlayoffRound"]] = relationship(
        back_populates="playoff_config", cascade="all, delete-orphan"
    )
...
class PlayoffRound(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "playoff_config_id", "round_number", name="uq_playoff_round_config_number"
        ),
    )

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    playoff_config_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("playoff_configs.id"), nullable=False
    )
    ...
    draft_resolved_at: Mapped[datetime | None] = mapped_column(
    ...
    pods: Mapped[list["PlayoffPod"]] = relationship(
        back_populates="playoff_round", cascade="all, delete-orphan"
    )
...
class PlayoffPod(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "playoff_round_id", "bracket_position", name="uq_playoff_pod_round_position"
        ),
    )
    ...
    members: Mapped[list["PlayoffPodMember"]] = relationship(
        back_populates="pod", cascade="all, delete-orphan"
    )
    picks: Mapped[list["PlayoffPick"]] = relationship(
        back_populates="pod", cascade="all, delete-orphan"
    )
    draft_preferences: Mapped[list["PlayoffDraftPreference"]] = relationship(
        back_populates="pod", cascade="all, delete-orphan"
    )
...
class PlayoffPodMember(Base):
    ...
    draft_preferences: Mapped[list["PlayoffDraftPreference"]] = relationship(
        back_populates="pod_member", cascade="all, delete-orphan"
    )
...
class PlayoffPick(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            ...
            "pod_member_id",
            "draft_slot",
            name="uq_playoff_pick_pod_member_slot",
        ),
    )
